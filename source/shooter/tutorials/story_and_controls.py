    ...
    def update(self, key):
        if self.check_input_to_advance(key):
            if self.showing == "1":
                ...
                self.window.show("Looks like the jump drive is offline. I'll need to rescue {0} escape pods so we can repair the ship so we can jump to safety. They should be easy to spot with their blinking SOS lights.".format(max_pods), "captain")
            elif self.showing == "2":
                self.showing = "3"
